...
(defn state-put [m ks v]
  (if (or (= 1 (count ks)) (= ks nil))
    (assoc m (first ks) v)
    (assoc m (first ks) (state-put (get m (first ks)) (rest ks) v))))
...
(defn state-remove [m ks]
  (if (or (= 1 (count ks)) (= ks nil))
    (dissoc m (first ks))
    (let [val (state-remove (get m (first ks)) (rest ks))] ;;new set
      (if (empty? val)
        (dissoc m (first ks)) ;;if empty remove
        (assoc m (first ks) val))))) ;;if not empty, replace
